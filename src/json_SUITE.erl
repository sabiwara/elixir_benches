...
encode(Term) -> iolist_to_binary(faster_json:encode(Term)).

encode_escape_all(Term) ->
    Encode = fun
        (Binary, _) when is_binary(Binary) -> faster_json:encode_binary_escape_all(Binary);
        (Other, Encode) -> faster_json:encode_value(Other, Encode)
    end,
    iolist_to_binary(faster_json:encode(Term, Encode)).

encode_checked(Term) ->
    Encode = fun
        (Map, Encode) when is_map(Map) -> faster_json:encode_map_checked(Map, Encode);
        (Other, Encode) -> faster_json:encode_value(Other, Encode)
    end,
    iolist_to_binary(faster_json:encode(Term, Encode)).
...
encode_proplist(Term) ->
    Encode = fun
        ([{_, _} | _], Encode) -> faster_json:encode_key_value_list(Term, Encode);
        (Other, Encode) -> faster_json:encode_value(Other, Encode)
    end,
    iolist_to_binary(faster_json:encode(Term, Encode)).
...
encode_proplist_checked(Term) ->
    Encode = fun
        ([{_, _} | _], Encode) -> faster_json:encode_key_value_list_checked(Term, Encode);
        (Other, Encode) -> faster_json:encode_value(Other, Encode)
    end,
    iolist_to_binary(faster_json:encode(Term, Encode)).
...
format(Term) -> iolist_to_binary(faster_json:format(Term)).
format(Term, Arg) -> iolist_to_binary(faster_json:format(Term, Arg)).
...
test_format_proplist(_Config) ->
    Formatter = fun({kvlist, KVList}, Fun, State) ->
                        faster_json:format_key_value_list(KVList, Fun, State);
                   ({kvlist_checked, KVList}, Fun, State) ->
                        faster_json:format_key_value_list_checked(KVList, Fun, State);
                   (Other, Fun, State) ->
                        faster_json:format_value(Other, Fun, State)
                end,
...
test_decode_numbers(_Config) ->
    ...
    ?assertError({invalid_byte, $1}, faster_json:decode(<<"01">>)),
    ?assertError({invalid_byte, $.}, decode(<<".1">>)),
...
test_decode_api(_Config) ->
    ...
    {Decoded, Acc, <<>>} = faster_json:decode(Data, {[], 0}, Decoders),
    ?assertEqual({[], 25}, Acc),
...
test_decode_api_stream(_Config) ->
    ...
    {12345, ok, B1} = faster_json:decode(ews(~# 12345 "foo" #), ok, #{}),
    <<" \s\t\r\n", _/binary>> = B1,
    {<<"foo">>, ok, <<>>} = faster_json:decode(B1, ok, #{}),
...
stream_decode(Str) ->
    {R1, [], <<>>} = byte_loop(Str),
    case faster_json:decode(Str) of
        R1 ->
            ok;
        R2 ->
...
multi_stream_decode(Strs) ->
    ...
    case faster_json:decode(Strs, [], #{}) of
        {R1, [], ContBin} ->
...
byte_loop(<<>>, State, _Bytes) ->
    faster_json:decode_continue(end_of_input, State).
...
test_file(yes, File, Data) ->
    ...
    ?assertEqual(Parsed, decode(iolist_to_binary(faster_json:format(Parsed))), File);
