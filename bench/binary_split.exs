...
Benchee.run(
  %{
    "simple list" => fn string ->
      :binary.split(string, simple_list, [:global, :trim_all])
    end,
    "simple pattern" => fn string ->
      :binary.split(string, simple_pattern, [:global, :trim_all])
    end,
    "complex list" => fn string ->
      :binary.split(string, complex_list, [:global, :trim_all])
    end,
    "complex pattern" => fn string ->
      :binary.split(string, complex_pattern, [:global, :trim_all])
    end
  },
  inputs: [
    {"small", small},
